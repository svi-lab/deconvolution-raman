...
Third plot: the heatmap of the mixing coefficients (shows the abundance of each component troughout the map)
            when you double-click on a pixel on this map, it will pop-up another plot 
            showing the spectra recorded at this point, together with the contributions of each component
'''
...
initialization = {'SliceValues':[850, 1250], # Use None for taking the extremes
...
"origins" is a pandas dataframe giving detail on each point in the map scan (time of measurement, _coordinates and some other info).
...
you should be much better off if you eliminate the cosmic rays beforehand using WiRE'''
...
elif filename == 'Data/Sirine_siO21mu-plr-532nm-obj100-2s-p100-slice--10-10.wdf':
    _slice_to_exclude = np.index_exp[[326, 700, 702, 1019, 1591, 1717, 2254, 
                                      3220, 3668, 3939, 5521, 6358, 6833, 6967, 
                                      7335, 7864, 8426, 10538, 10572, 11809]]
    _slice_replacement = np.index_exp[[327, 701, 703, 1020, 1592, 1718, 2255, 
                                       3221, 3669, 3940, 5522, 6359, 6832, 6968, 
                                       7336, 7865, 8427, 10539, 10573, 11808]]
else:
...
pca.n_components = min(10, _n_points, len(sigma_kept))
'''Note that the choice of min 10 components is completely arbitrary
...
spectra_cleaned = deconvolution.clean(sigma_kept, spectra_denoised, mode='area')
...
components, mix, nmf_reconstruction_error = deconvolution.nmf_step(spectra_cleaned, _n_components, init='nndsvda')
_basic_mix = pd.DataFrame(np.copy(mix), columns = [f"mixing coeff. for the component {l}" for l in np.arange(mix.shape[1])])
_end = time()
...
for _z in range(_n_components):
    _comp_area[_z] = integrate.trapz(components[_z]) # area beneath each component
    components[_z] /= _comp_area[_z] # normalizing the components by area
    mix[:,_z] *= _comp_area[np.newaxis,_z] # renormalizing the mixture coefficients
...
fi, _ax = plt.subplots(int(np.floor(np.sqrt(_n_components))), int(np.ceil(_n_components/np.floor(np.sqrt(_n_components)))))
if _n_components > 1:
    _ax = _ax.ravel()
else:
    _ax = [_ax]
...
fig, _ax = plt.subplots(int(np.floor(np.sqrt(_n_components))), 
                        int(np.ceil(_n_components/np.floor(np.sqrt(_n_components)))), 
...
if _n_components > 1:
    ...
    _ax = [_ax]
def onclick(event):
    '''Double-clicking on a pixel will pop-up the (cleaned) spectrum corresponding to that pixel, as well as it'_s deconvolution on the components
    and again the reconstruction for visual comparison'''
    if event.inaxes:
#        a = event.inaxes
#        for ii, axax in enumerate(_ax):
#            if axax == a:
#                iii = ii
        ...
        if event.dblclick:
            ...
            for k in range(_n_components):
                aa.plot(sigma_kept, components[k]*mix[broj][k], 
                        ...
                        label=f'Component {k} contribution ({mix[broj][k]*100:.1f}%)')
...
#    _ax[_i].set_aspect(_s_y/_s_x)
    _ax[_i].set_title(f'Component {_i}', color=color_set.to_rgba(_i), fontweight='extra bold')
